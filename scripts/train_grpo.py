#!/usr/bin/env python3
"""Train official ranking GRPO on one local CUDA GPU."""
...
from minionerec.training.grpo_training import (  # noqa: E402
    GRPOTrainingConfig,
    build_grpo_components,
)


def build_parser() -> argparse.ArgumentParser:
    """Build the single-GPU GRPO command-line parser."""

    parser = argparse.ArgumentParser(
        description=(
            "Full-parameter BF16 ranking-GRPO for the fixed MiniOneRec "
            "commit. All models and datasets must already exist locally."
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Run two optimizer steps with no evaluation or intermediate "
            "checkpoint"
        ),
    )
    return parser
...
def _training_config(arguments: argparse.Namespace) -> GRPOTrainingConfig:
    return GRPOTrainingConfig(
        ...
        gradient_accumulation_steps=(
            ...
            else arguments.gradient_accumulation_steps
        ),
...
def _training_parameters(
    ...
    return {
        ...
        "unique_prompts_per_update": config.unique_prompts_per_update,
...
def main() -> int:
    """Run GRPO training and persist the final model and statistics."""
    ...
    try:
        ...
        config = _training_config(arguments)
        started = time.perf_counter()
        components = build_grpo_components(config)
        preflight = {
